            ...
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                ...
                if include_root_block:
                    if output_stride is not None:
                        ...
                        output_stride /= 4
                    # We do not include BN or activ_func in conv1 because the 1st residual unit will perform these.
                    with slim.arg_scope([slim.conv2d], activation_fn=None, normalizer_fn=None):
                        net = resnet_utils.conv2d_same(net, 64, 7, stride=2, scope='conv1')
                    net = slim.max_pool2d(net, [3, 3], stride=2, scope='pool1')
                    net = slim.utils.collect_named_outputs(end_points_collection, 'pool2', net)
                net = resnet_utils.stack_blocks_dense(net, blocks, output_stride)
                # Pre-activation variant does not have BN or activ_func in the residual unit output. See Appendix of [2].
...
def test():
    resnet = resnet_v2_50(tf.random_normal([1, 32, 32, 3]), num_classes=10, is_training=True, output_stride=8)
    print(resnet)
