...
def convert2prob(h, scope):
    with tf.variable_scope(scope):
        px = tf.nn.relu(tf.nn.sigmoid(h)-1e-6)  # [0, 1] => [-1e-6, 1-1e-6] => [0, 1)
        # PC_k ~ B(9, (1-px))
        PC1 = tf.pow(1-px, 9)  # (1-px)^9
        PC2 = PC1 * px * 9./(1-px)  # 9 * px *(1 - px)^8
        PC3 = PC2 * px * 8./(2*(1-px))  # 9*8/2 * px^2 * (1-px)^7
        PC4 = PC3 * px * 7./(3*(1-px))
        PC5 = PC4 * px * 6./(4*(1-px))
        PC6 = PC5 * px * 5./(5*(1-px))
        PC7 = PC6 * px * 4./(6*(1-px))
        ...
        PC = tf.concat([PC1, PC2, PC3, PC4, PC5, PC6, PC7, PC8, PC9, PC10], axis=1)
        return PC
...
def network(x, scope, reuse=False, phase=True):
    """ x: input
        scope: name of variable_scope on the network level
        reuse: whether the layers are set on reuse mode
        phase: whether the BN layers are in training mode"""
    with tf.variable_scope(scope):
        # first layer: 4096 -> 512
